...
export class ScriptedPlanner implements Planner {
  ...
  constructor(
    ...
    /** 支払い対象の選び方。rail 指定 or endpoint 直指定。 */
    private readonly target: { rail?: string; endpoint?: string },
  ) {}
  ...
  async next(toolResults: Array<{ id: string; result: unknown }>): Promise<PlannerStep> {
    ...
    if (this.step === 2) {
      ...
      if (!endpoint) {
        return { calls: [], text: "候補が見つからなかった", done: true };
      }
      return {
        calls: [
          {
    ...
    const last = toolResults[toolResults.length - 1]?.result as { status?: string } | undefined;
    return { calls: [], text: `完了: status=${last?.status ?? "unknown"}`, done: true };
  }
}
